...
- `pytest -q`: 179 passed (includes the end-to-end form on a real dispatcher)
...
| The same report added: «zayezd haliyam 3 oktabr ko'rsatyapti aslida 2» | The «Заезд участников — …» line (and the ticket stub) print the tenant's «Дата мероприятия», which is the *arrival* date — but the field read like the event/show date, so a value naming the 3rd (the show day) landed in the arrival line | The panel labels now say «Дата заезда участников (RU/UZ)» / «Ishtirokchilar kirish sanasi», the hint names the message the value appears in, and on startup an SPL arrival date that names 3 October (any spelling: `3 октября`, `03.10.2026`, `3-oktyabr`, including `2-3 октября`) is corrected to the confirmed `02 октября 2026 с 17:00 до 22:00` with a warning in the log. A value that is *not* the 3rd is treated as deliberate and left alone; other tenants are never touched. `/diag` now prints the exact date/venue/instruction the bot will use |
