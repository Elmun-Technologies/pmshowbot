...
- Live ticket preview (`/ticket-assets/preview.png`) with current logos,
  built from the tenant's newest application (neutral sample when there are
  none).
- The **application page** shows the participant's *actual* ticket
  (`/t/<slug>/application/{id}/ticket.png` — same render as delivery) with
  full-size / download links and the 🎫 resend button: the resend answers
  instantly and delivers in the background, the page auto-refreshes until the
  outcome is shown (a failure carries the exact readable reason).
...
- `pytest -q`: 194 passed (includes the end-to-end form on a real dispatcher)
...
| «admin panelda preview qilib biletni ko'rib bo'lmayapti / biletini qayta yuborib bo'lmayapti» — the panel ticket section could not be used | The application page had **no preview of the participant's actual ticket** (the ticket-assets page only showed a generic sample with hard-coded "Adrenaline Drift"/Russian copy), and the 🎫 resend click awaited the **whole delivery** — render (up to ~90 s per attempt on a busy render pool) plus a multi-megabyte upload — so on a busy day the click looked dead for minutes; the failure reason then came back double-URL-encoded in a query parameter (percent gibberish) | The application page now shows the participant's real ticket: `GET /t/<slug>/application/{id}/ticket.png` renders it through the delivery path (hero photo → gradient fallback, brief in-memory cache, full-size / download links). The resend answers with a redirect at once and delivers in a background task (the same rule as /approve): the page auto-refreshes while in flight, a second click does not duplicate the send, and the outcome — including the exact readable failure reason in RU/UZ — is shown on the page. The ticket-assets sample is built from the tenant's newest application instead of foreign copy |
...
| «Prinyat bosilsa judayam sekin ishlayapti, keyin zayafka uje obrabotana deyapti» — Accept stays spinning, the second tap says "already processed", and the participant gets no ticket | The callback handler awaited the **whole delivery**: the SQLite decision, the notification, the ticket render and a ~3 MB PNG upload (6.63 s on a 500 KB/s uplink, far worse on an event-grade phone connection). Telegram keeps the button spinning until `query.answer`, so the moderator tapped again — by then the row *was* approved, hence «уже обработана». The PNG poster is photographic (~2.8 MB for a real phone photo) while Telegram re-encodes photos anyway | The decision is the only awaited step: `claim_approval` (one `BEGIN IMMEDIATE` write, milliseconds) → card marked «🎫 Билет отправляется участнику…» + buttons removed → `query.answer`. Notification, render, upload, share hint and the sheet append run in a background task (`decisions.spawn`, kept referenced) which then edits the card to «🎫 Билет отправлен участнику» or «🎫 Билет НЕ ушёл: … — /ticket 123». Posters over 1.2 MB upload as a quality-88 JPEG first (2.77 MB → 0.54 MB), the render has its own small pool so a queue of Drive uploads cannot starve it, and the panel too answers before the delivery. Measured: answer 6.63 s → 0.01 s, ticket 6.63 s → 0.74 s (`diagnostics/perf_accept.py`) |
| The same report added: «zayezd haliyam 3 oktabr ko'rsatyapti aslida 2» | The «Заезд участников — …» line (and the ticket stub) print the tenant's «Дата мероприятия», which is the *arrival* date — but the field read like the event/show date, so a value naming the 3rd (the show day) landed in the arrival line | The panel labels now say «Дата заезда участников (RU/UZ)» / «Ishtirokchilar kirish sanasi», the hint names the message the value appears in, and on startup an SPL arrival date that names 3 October (any spelling: `3 октября`, `03.10.2026`, `3-oktyabr`, including `2-3 октября`) is corrected to the confirmed `02 октября 2026 с 17:00 до 22:00` with a warning in the log. A value that is *not* the 3rd is treated as deliberate and left alone; other tenants are never touched. `/diag` now prints the exact date/venue/instruction the bot will use  *(Superseded — see the next row: the startup correction is gone.)* |
| «После одобрения приходит сообщение с неправильными датами, временем», «при отклонении — неправильное время» (September 2026) | Every startup re-seeded the SPL arrival date and instruction (and rewrote a «3 октября» typed by the team), so a correction made in the panel was undone by the next deploy — the panel was not the source of truth | Nothing about the SPL schedule is seeded any more (only the venue «Tashkent INDEX»). A one-time migration per SPL tenant (`app_meta` key `spl_messages_2026_09:<id>`) clears values an earlier build wrote, keeps hand-typed ones and clears the guest date, so the rejection carries no time. The panel also has **«Тексты сообщений участнику»**: full approval/rejection texts (RU/UZ) with `{number} {name} {plate} {direction} {event} {date} {venue} {channel}`; an empty field falls back to the generated text, a typo in a placeholder is printed as typed and never breaks delivery |
| «Категории SPL Avtozvuk обновить» + «выбирать до 4 категорий» | Only 4 grouped categories, one choice | 16 categories (SPL Sport Багажник 2К … SPL Game 149.99). On boot the 4 old ones are switched off (rows kept for old applications) and missing new ones added; hand-added categories are untouched. The participant picks up to 4: after each pick the menu reopens without the picked ones, «Готово ✅» finishes, the 4th pick finishes automatically. Stored as `"A; B; C"` in `applications.direction` (the card and messages show a bullet list, the ticket a comma list) |
| «Spl Show» instead of «SPL Show» | The tenant name was typed that way in the panel | `normalize_spl_brand` fixes the name on boot (once), on every panel save, and wherever the name is printed |
| A second Accept tap only said «Эта заявка уже обработана» — which looked like the first tap was lost | The alert carried no information about the decision that was already stored | The alert names it: «Уже принята — №12 (@moderator).» / «Уже отклонена (@moderator).» |
